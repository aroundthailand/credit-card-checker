...
const validateCred = creditCardsArray => {
    ...
    for (let i = creditCardsArray.length-1; i>=0; i = i-2) {
        sum += creditCardsArray[i];
        if(i !== 0) {
            let doubledDigit = creditCardsArray[i-1] * 2;
            if(doubledDigit > 9) {
                doubledDigit -= 9;
            }
            sum += doubledDigit;
...
const idInvalideCardCompanies = cardsArray => {
    ...
    return invalidCompaniesCard;
}

console.log(idInvalideCardCompanies(findInvalidCards(batch)));